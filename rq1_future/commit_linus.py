...
commit_lines = [i[0] for i in commit_data]

json_file = 'commit_lines3.json'
with open(json_file, 'w') as file:
    json.dump(commit_lines, file, indent=4)
...
for i, commit_id in enumerate(commits_only_in_first):
    ...
    try:
        ...
        # Count the number of code line additions and deletions
        additions = 0
        deletions = 0
        for file_diff in diff:
            if file_diff.a_path.endswith('.rs'):  # Consider only Python files, modify if needed
                if file_diff.diff is None:
                    continue
                if file_diff.a_blob is None:
                    continue
                diff_text = file_diff.a_blob.data_stream.read().decode('utf-8')  # Convert bytes to string
                additions += diff_text.count('+') - diff_text.count('+++')
                deletions += diff_text.count('-') - diff_text.count('---')
        sums += (additions + deletions)
        print(sums)
        ns.append(sums)
        commit_stats[commit_id] = {
            'additions': additions,
            'deletions': deletions,
...
for i in range(0, len(commit_data)):
    commit_count_values.append(commit_stats[commit_lines[i]]['sum'])

commit_count_values = ns
...
# Show the plot
image_file = 'commit_lines_sum_dev.png'
